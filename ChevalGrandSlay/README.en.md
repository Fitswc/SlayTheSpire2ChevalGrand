...
- Template Wiki (Rider-first): [Chinese Home](https://github.com/alkaid616/ChevalGrandSlay/wiki/Home) | [English Home](https://github.com/alkaid616/ChevalGrandSlay/wiki/Home-EN).
...
`dotnet new ritsulibmod -n MyMod` generates a project called `MyMod` and renames `ChevalGrandSlay`, sample class names, sample resource file names, resource folders, manifest names, namespaces, and localization IDs to match the new name.

### Option B: manual copy

1. Copy the whole directory and rename it to your mod name.
2. Edit `ChevalGrandSlay.json` and update `id`, `name`, `author`, and `description`.
3. Edit `ChevalGrandSlayCode/Entry.cs` and update `ModId`.
4. For a full rename, also update the project name and namespace in `.csproj`, `.sln`, and `project.godot`.
5. Rename the resource directory `ChevalGrandSlay/` to your `ModId`, then update the related `Entry.ResPath` paths in code.
...
> **`PackageReference` in `.csproj` only controls compile-time resolution; `dependencies` in `ChevalGrandSlay.json` is what the game loader checks at runtime. The template syncs the `STS2-RitsuLib` dependency version to the resolved NuGet version during build, but `min_game_version` still needs manual review.**
...
1. After build, confirm `dependencies[STS2-RitsuLib].version` in `ChevalGrandSlay.json` has been synced to the resolved `STS2.RitsuLib` version.
...
| Command | Behavior |
|---|---|
| `dotnet build .\ChevalGrandSlay.csproj` | Full build: compile + `CopyMod` + `ExportPCK` |
...
- **`CopyMod`**: copies the DLL and manifest to the game's `mods/ChevalGrandSlay` directory.
...
```text
ChevalGrandSlay/
├── ChevalGrandSlayCode/   # C# source
├── ChevalGrandSlay/       # Godot resources, localization, and placeholder scenes
├── ChevalGrandSlay.csproj
├── ChevalGrandSlay.json   # Mod manifest
├── project.godot
└── local.props.template
```

`res://ChevalGrandSlay/...` is the Godot/PCK resource path, mapping to the repository resource directory `ChevalGrandSlay/`, **not** to the C# namespace. When you create a project from the NuGet template, these directory names, file names, and namespaces are renamed consistently to match the new mod name.
...
| Property | Value |
|---|---|
| Type | `ChevalGrandSlayCharacter` |
| Expected ID | `CHEVAL_GRAND_SLAY_CHARACTER_CHEVAL_GRAND_SLAY_CHARACTER` |
| Starter deck | 4 × `ChevalGrandSlayStrike`, 4 × `ChevalGrandSlayDefend`, 1 × `ChevalGrandSlayRelic` |
...
|---|---|---|
| `ChevalGrandSlayStrike` (attack) | character card pool | `CHEVAL_GRAND_SLAY_CARD_CHEVAL_GRAND_SLAY_STRIKE` |
| `ChevalGrandSlayDefend` (skill) | character card pool | `CHEVAL_GRAND_SLAY_CARD_CHEVAL_GRAND_SLAY_DEFEND` |
| `ChevalGrandSlayRelic` | `ChevalGrandSlayRelicPool` | `CHEVAL_GRAND_SLAY_RELIC_CHEVAL_GRAND_SLAY_RELIC` |

### Static placeholder assets

**Images** (`res://ChevalGrandSlay/images/...`):

- `cards/ChevalGrandSlayStrike.png`, `cards/ChevalGrandSlayDefend.png`: sample card art.
- `relics/ChevalGrandSlayRelic.png`: sample relic icon.
- `characters/ChevalGrandSlay_character_*.png`: character icons, select art, map marker, and energy icons.

**Scenes** (`res://ChevalGrandSlay/scenes/characters/...`):

| Scene | Purpose | Placeholder structure |
|---|---|---|
| `ChevalGrandSlay_character.tscn` | Combat character | `%Visuals`, `%Bounds`, `%IntentPos`, `%CenterPos`, `%TalkPos` |
| `ChevalGrandSlay_energy_counter.tscn` | Energy counter | `%EnergyVfxBack`, `%Layers`, `%RotationLayers`, `%EnergyVfxFront`, `Label` |
| `ChevalGrandSlay_merchant.tscn` | Merchant | — |
| `ChevalGrandSlay_rest_site.tscn` | Rest site | `%ControlRoot`, `%SelectionReticle`, `%Hitbox`, `%ThoughtBubbleRight`, `%ThoughtBubbleLeft` |
| `ChevalGrandSlay_character_select_bg.tscn` | Character select background | — |
...
`ChevalGrandSlay.json` is the mod manifest. The game loader reads it at startup to identify the mod, check dependencies, and decide whether to load. Full example:

```json
{
  "id": "ChevalGrandSlay",
  "name": "ChevalGrandSlay",
  "pck_name": "ChevalGrandSlay",
  "author": "Author",
